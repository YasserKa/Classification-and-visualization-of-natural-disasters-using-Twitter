...
downtime. \citeauth{fengExtractionPluvialFlood2018} only fetch geotagged tweets and then filter 
...
interest in different languages. \citeauth{debruijnGlobalDatabaseHistoric2019b} uses over 40
...
For common tasks such as translation an encoder-decoder architecture is needed, where the encoder encodes the
...
\citeauth{petersenIdentificationExplorationExtreme2021} fine-tunes a pre-trained \ac{ULMFit} model
...
\citeauth{alamFloodDetectionTwitter2020} fine-tunes a pre-trained
\ac{BERT}\cite{devlinBERTPretrainingDeep2019}  model that works on one language with an accuracy of
0.853, and \citeauth{debruijnGlobalDatabaseHistoric2019b} uses a multilingual model with 0.8
F1-score.
...
\citeauth{heusingerDimensionalityReductionContext2022} uses random projection to reduce the
dimensions of tweets to predict their hashtags, making them easier to search on Twitter.
\citeauth{omuyaSentimentAnalysisSocial2022} extracts features from social media using Principal
